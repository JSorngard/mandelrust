...
pub fn run(config: Config) -> Result<(), Box<dyn Error>> {
    ...
    let xresolution = aspect_ratio*(yresolution as f64);
    println!("The real part of the center point is {}, and the imaginary part is {}", center_re, center_im);
    println!("The aspect ratio is {}, and the resolution is {}x{}", aspect_ratio, xresolution, yresolution);
    if save_result {
    ...
    //1. Determine what function to use: image of animation
    //2. Call render() with the required parameters
    //3. Save the result
...
pub fn iterate(c_re: f64, c_im: f64, maxiterations: i64) -> f64 {
    ...
    (maxiterations - iterations) as f64 - 4.0*f64::powf((z_re_sqr + z_im_sqr).sqrt(),-0.4)/(maxiterations as f64)
}
...
impl Config {
    ...
    pub fn new() -> Result<Config, &'static str> {
        ...
        let resolution: i64 = match resolution.trim().parse() {
            Ok(num) => num,
            Err(_) => return Err("Could not interpret RESOLUTION as an integer")
        };

        Ok(Config {center_re, center_im, aspect_ratio, resolution, save_result})
    }
}